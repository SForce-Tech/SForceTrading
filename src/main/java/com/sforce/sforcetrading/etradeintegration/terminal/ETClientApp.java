package com.sforce.sforcetrading.etradeintegration.terminal;
...
public class ETClientApp extends AppCommandLine {
	...
	protected Logger log = LoggerFactory.getLogger(ETClientApp.class);
	 ...
	 * log.debug(e);
	 * out.println(
	 * " Sorry we are not able to continue at this time, please restart the client.."
	 * );
	 * }
	 * }
	 */
	...
	public void getAccountList() {
		...
		try {
			...
			out.println(String.format("\n%20s %25s %25s %25s %25s\n", "Number", "AccountId", "AccountIdKey",
					"AccountDesc", "InstitutionType"));
			...
			try {
				...
				while (itr.hasNext()) {
					...
					if (acctStatus != null && !acctStatus.equals("CLOSED")) {
						...
						out.println(String.format("%20s %25s %25s %25s %25s\n", count, innerObj.get("accountId"),
								acctIdKey, innerObj.get("accountDesc"), innerObj.get("institutionType")));
						count++;
					}
				}
	...
	public void getBalance(String acctIndex) {
		...
		try {
			...
			out.println(String.format("%s\t\t\tBalances for %s %s%s", lineSeperator, accountId, lineSeperator,
					lineSeperator));
	...
	public void getPortfolio(String acctIndex) {
		...
		try {
			...
			while (acctItr.hasNext()) {
				...
				acctIdBuf.append(lineSeperator).append("\t\t Portfolios for ").append(accountId).append(lineSeperator)
				...
				while (itr.hasNext()) {
					...
					fmt.format(formatString.toString(), responseData[0], responseData[1], responseData[2],
							responseData[3], responseData[4], responseData[5], responseData[6]);
				}

			}
			out.println(acctIdBuf.toString());

			String titleFormat = new StringBuilder("%25s %25s %25s %25s %25s %25s %25s").append(System.lineSeparator())
					.toString();
				...
				if (quoteData != null) {
					...
					while (itr.hasNext()) {
						...
						if (mutualFund != null && mutualFund.get("changeClose") != null
							...
							String changeClosePercentage = (String) (mutualFund.get("changeClosePercentage"))
									.toString();
							out.println("Today's Change: " + changeClose + " (" + changeClosePercentage + "%)");
						}
	...
	public void getOrders(final String acctIndex) {
		...
		try {
			...
			if (response != null) {
				...
				acctIdBuf.append(lineSeperator).append("\t\t Orders for selected account index : ").append(acctIndex)
