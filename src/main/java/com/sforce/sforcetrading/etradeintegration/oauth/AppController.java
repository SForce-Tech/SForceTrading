	...
	public String invoke(Message message) throws ApiException {
		...
		if (!context.isIntialized() && message.getOauthRequired() == OauthRequired.YES) {
			...
			if (requestTokenService != null) {
				...
				requestTokenService.handleNext((Receiver) authorizationService);
				((Receiver) authorizationService).handleNext(accessTokenService);
				...
				if (!context.isIntialized()) {
					...
					throw new ApiException(500, "500",
							"Oauth initialization failure, only delayed quote request is available");
				} else {
