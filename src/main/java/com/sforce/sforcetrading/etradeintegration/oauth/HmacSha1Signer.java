...
public class HmacSha1Signer implements OAuthSigner {
	...
	private static final Logger logger = LoggerFactory.getLogger(HmacSha1Signer.class);
	...
	public String computeSignature(String signatureBaseString, SecurityContext context)
		...
		if (token != null) {
			key = StringUtils.isEmpty(token.getOauth_token_secret()) ? context.getResouces().getSharedSecret() + "&"
					: context.getResouces().getSharedSecret() + "&"
							+ OAuth1Template.encode(token.getOauth_token_secret());
		} else {
		...
		logger.debug(" HmacSha1Signer - signatureBaseString  " + signatureBaseString);
		...
		logger.debug(" computeSignature from HMAC " + result);
		return result;
	}
}
