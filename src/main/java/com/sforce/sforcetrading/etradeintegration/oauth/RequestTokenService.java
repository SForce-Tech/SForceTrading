...
public class RequestTokenService implements Receiver {
	...
	public boolean handleMessage(Message message, SecurityContext context) throws ApiException {
		...
		try {
			...
			params.computeOauthSignature(context.getResouces().getRequestTokenHttpMethod(),
					context.getResouces().getRequestTokenUrl());
		...
		if (nextReceiver != null)
			((RequestTokenService) nextReceiver).handleMessage(message, context);
		else
	...
	private String getURL() {
		// return
		// String.format("%s%s",oAuthProperties.get("BASE_URL"),oAuthProperties.get("TOKEN_URL"));
		return "";
	}
