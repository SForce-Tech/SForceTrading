package com.sforce.sforcetrading.etradeintegration.exception;
...
public class RestTemplateResponseErrorHandler extends DefaultResponseErrorHandler {
	...
	public void handleError(ClientHttpResponse response) throws IOException, ApiException {
		...
		if (((HttpStatus) response.getStatusCode()).series() == HttpStatus.Series.SERVER_ERROR ||
				((HttpStatus) response.getStatusCode()).series() == HttpStatus.Series.CLIENT_ERROR) {

			String xml = StreamUtils.copyToString(response.getBody(), Charset.defaultCharset());
			logger.warn(" Error Response " + xml);

			try {
				doc = loadXMLFromString(xml);
				logger.debug(" Documented created successfully");
			} catch (Exception e) {
				logger.error(" Error in parsing error xml", e);
				throw new ApiException(400, "client_error", "Generic Exception, please restart the client");
			}
			if (doc != null) {

				logger.error("Throwing ApiException....");
	...
	// <Error> <code>102</code> <message>Please enter valid Account
	// Key</message></Error>
