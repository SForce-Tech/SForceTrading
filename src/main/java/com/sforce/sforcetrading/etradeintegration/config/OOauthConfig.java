...
import com.sforce.sforcetrading.etradeintegration.exception.RestTemplateResponseErrorHandler;
import com.sforce.sforcetrading.etradeintegration.oauth.AccessTokenService;
import com.sforce.sforcetrading.etradeintegration.oauth.AppController;
import com.sforce.sforcetrading.etradeintegration.oauth.CustomRestTemplate;
import com.sforce.sforcetrading.etradeintegration.oauth.RequestTokenService;
...
import com.sforce.sforcetrading.etradeintegration.oauth.model.Signer;
...
@ComponentScan(basePackages = { "com.sforce.sforcetrading.*" })
public class OOauthConfig extends WebSecurityConfiguration {
	...
	/*
	 * Rest template that is able to make REST requests with the oauth credentials
	 ...
	 * Application uses oauth_token & oauth_token_secret to access protected
	 * resources that is available via oauth handshake.
	 */
	@Bean
	...
	/*
	 * Bean will fetch REALTIME quotes if the user has authorized the client with
	 * oauth handshake.
	 * Bean will fetch DELAYED quotes if the user has not done the oauth handshake,
	 * requires consumerKey as query param
	 */
	@Bean
	...
	/*
	 * Resource object used within client to retrieve oauth related properties and
	 * urls
	 */
	public Resource oauthResouce() {
		...
		// the authorization URL does not prompt for confirmation but immediately
		// returns an OAuth verifier
	...
	/*
	 * Resource object used within client to retrieve api related properties and
	 * urls
	 */
	@Bean
	...
	/*
	 * Bean to send singed request for a Request Token. On success, will grant
	...
	/*
	 * Bean to send user to authorize url using oauth token and promt the user to
	 * authorize/grant access. On authorize, bean will
	...
	/*
	 * Bean to exchange Request Token / Verifier for Access Token and signed
	 * request. On success, grants Access Token & Token Secret
	 */
	@Bean
